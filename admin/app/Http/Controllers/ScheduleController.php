...
class ScheduleController extends Controller
{
    ...
    public function getScheduleList()
    {
        ...
        return view('PageAdmin.schedule.schedule_list', ['schedule' => $schedule]);//trả về view hiển thị danh sách các lịch chiếu,đồng thời truyền một biến schedule sang view đó để truy xuất tạo danh sách lịch chiếu động bên view hiển thị danh sách lịch chiéu
    }
    //Hàm này sẽ trả xử lý việc xóa một lịch chiếu.
    public function ScheduleDelete($id, $marap, $giochieu)
    {
    ...
    public function postScheduleAdd(Request $request)
    {
        if ($request->idMovie == 0 || $request->idRap == 0) {//Kiểm tra xem người dùng đã chọn phim hoặc chọn rạp chưa nếu chưa thì nó trả về view thêm một lịch chiếu và thông báo lỗi.
            return redirect('PageAdmin/schedule/schedule_add')->with('loi', 'Bạn chưa chọn phim hoặc chọn rạp để trình chiếu');
        }
        ...
        if ($request->Day == 0 || $request->Month == 0 || $request->Year == 0) {//Kiểm tra người dùng đã chọn ngày tháng năm cho lịch chiếu hay chưa.
            return redirect('PageAdmin/schedule/schedule_add')->with('loi', 'Bạn chưa chọn ngày tháng năm để chiếu phim');//Nếu người dùng chưa chọn thì sẽ thông báo cho người dùng biết :"Bạn chưa chọn ngày tháng năm để chiếu phim")
        }
        ...
        if ($startday < $daycurrent) {//Kiểm tra người dùng có nhập ngày bắt đầu lịch chiếu có lớn hơn ngày hiện tại không.
            ...
            return redirect('PageAdmin/schedule/schedule_add')->with('loi', 'Ngày chiếu không thể nhỏ hơn ngày hiện tại');
        }
    ...
    public function getScheduleEdit($id, $marap, $giochieu){
        $movie = movie::all();
        $rap = rap::all();
        $schedule = schedule::where('MAPHIM', '=', $id)->where('MARAP', '=', $marap)->where('GIOCHIEU', '=', $giochieu)->get();
    ...
    public function postScheduleEdit(Request $request, $id, $marap, $giochieu){
        $this->validate($request, [
                'txtStartTime' => 'required',
                'txtPrice' => 'required',
            ], [
        ...
        if ($request->Day == 0 || $request->Month == 0 || $request->Year == 0) {//Nếu người dùng điều chỉnh lịch chiếu mà không sữa ngày bắt đầu chiếu thì chúng chỉ sữa GIOCHIEU va PRICE
            ...
            if ($startday < $daycurrent) {//Kiểm tra người dùng có nhập ngày bắt đầu lịch chiếu có lớn hơn ngày hiện tại không.
                //Nếu người dùng nhập nhỏ hỏn ngày hiện tại nó sẽ thông báo cho người dùng " Ngày chiếu không thể nhỏ hơn ngày hiện tại "
                return redirect('PageAdmin/schedule/schedule_edit/' . $id . '/' . $marap . '/' . $giochieu)->with('loi', 'Ngày chiếu không thể nhỏ hơn ngày hiện tại');
            }
        ...
        return redirect('PageAdmin/schedule/schedule_edit/' . $id . '/' . $marap . '/' . $request->txtStartTime)->with('thongbao', 'Bạn đã sữa thành công');
    }
}
