        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-lg-7" style="padding-bottom:120px">
                    ...
                    @endif
                    <form action="PageAdmin/schedule/schedule_edit/@foreach($schedule as $sche)
                                {{$sche['MAPHIM'].'/'.$sche['MARAP'].'/'.$sche['GIOCHIEU']}}
                            @endforeach
                        ...
                        <div class="form-group">
                            ...
                            @foreach($schedule as $sche)
                                {{$sche['MAPHIM']}}
                            @endforeach
                            <select class="form-control" name="idMovie" disabled>
                                <option value="0">Please Choose Movie</option>
                                @foreach($movie as $value)
                                    <option
                                            @foreach($schedule as $sche)
                                                @if($value->MAPHIM==$sche['MAPHIM'])
                                                    {{"selected"}}
                                                @endif
                                            @endforeach
                                            value="{{$value->MAPHIM}}">{{$value->TENPHIM}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Choose Cinema</label>
                            <select class="form-control" name="idRap" disabled>
                                <option value="0">Please Choose Cinema</option>
                                @foreach($rap as $value)
                                    <option
                                            @foreach($schedule as $sche)
                                                @if($value->MARAP==$sche['MARAP'])
                                                    {{"selected"}}
                                                @endif
                                            @endforeach
                                            value="{{$value->MARAP}}">{{$value->TENRAP}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <button type="submit" class="btn btn-default">Schedule Edit</button>
