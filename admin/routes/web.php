...
Route::get("PageAdmin/login",'UserController@get_Admin_Login');
Route::post("PageAdmin/login",'UserController@post_Admin_Login');
Route::get('PageAdmin/Logout','UserController@logout_PageAdmin');
...
Route::group(['prefix'=>'PageAdmin','middleware'=>'adminLogin'],function (){
   ...
   Route::group(['prefix'=>'category'],function (){
       ...
       //Các controller sẽ nằm trong thư mục App\Http\Controller.
        Route::get('category_list','CategoryController@get_Category_List')->name('get_Category_List');//Khi gọi đến route này nó sẽ vào Controller có tên là CategoryController và nó sẽ vào hàm get_Category_List để xử lý sau đó trả về cho người dùng một view.
        Route::get('category_add','CategoryController@get_Category_Add')->name('get_Category_Add');//Khi gọi đến route này nó sẽ vào Controller có tên là CategoryController và nó sẽ vào hàm get_Category_Add để xử lý sau đó trả về cho người dùng view để thêm một loại phim.
        Route::post('category_add','CategoryController@post_Category_Add')->name('post_category_add');//Khi gọi đến route này nó sẽ xữ lý thêm một phim vào database.
        Route::get('category_edit/{id}','CategoryController@get_Category_Edit');//Khi gọi đến route này nó sẽ vào Controller có tên là CategoryController và nó sẽ vào hàm get_Category_Edit để xử lý sau đó trả về cho người dùng view để sửa thông tin một loại phim.
        Route::post('category_edit/{id}','CategoryController@post_Category_Edit');//Khi gọi đến route này nó sẽ xử lý việc sữa một loại tin.
        Route::get('category_delete/{id}','CategoryController@get_Category_Delete')->name('get_category_delete');//KHi gọi route này nõ chuyển đến Controller có tên là CategoryController và nó sẽ gọi hàm get_Category_Delete để xử lý việc xóa một loại phim.
   });
    ...
    Route::group(['prefix'=>'user'],function (){
        Route::get('user_list','UserController@get_User_List');
        Route::get('user_add','UserController@get_User_Add');
        Route::post('user_add','UserController@post_User_Add');
        Route::get('user_edit/{id}','UserController@get_User_Edit');
        Route::post('user_edit/{id}','UserController@post_User_Edit');
        Route::get('user_delete/{id}','UserController@get_User_Delete');
    });
