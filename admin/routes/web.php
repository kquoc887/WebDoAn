...
Route::group(['prefix'=>'PageAdmin', 'middleware' => 'adminLogin'], function() {
   ...
   Route::group(['prefix' => 'category'], function() {
    ...
    Route::group(['prefix' => 'movie'], function() {
        Route::get('movie_list', 'MovieController@getMovieList')->name('movie_list');//Khi gọi đến route này nó sẽ vào Controller có tên là MovieController(nằm trong thư mục và nó sẽ vào hàm get_Movie_List để xử lý sau đó trả về cho người dùng một view hiển thị danh sách các phim.
        Route::get('movie_add', 'MovieController@getMovieAdd');//Khi gọi đến route này nó vào Controller có tên là MovieController và nó sẽ vào hàm get_Movie_Add để xử lý trả về view để người dùng có thể thêm một phim.
        Route::post('movie_add', 'MovieController@postMovieAdd');//Khi gọi đến route này nó vào Controller có tên là MovieController và nó sẽ vào hàm post_Movie_Add để xử lý việc thêm một phim.
        Route::get('movie_edit/{id}', 'MovieController@getMovieEdit');//Khi gọi đến route này nó vào Controller có tên là MovieController và nó vào hàm get_Movie_Edit để xử lý và trả về cho người dùng giao diện sữa một phim.
        Route::post('movie_edit/{id}', 'MovieController@postMovieEdit');//Khi gọi đến route này nó vào Controller có tên là MovieController và nó sẽ vào hàm post_Movie_Edit để xử lý việc sữa thông tin một phim
        Route::get('movie_delete/{id}', 'MovieController@getMovieDelete')->name('get_movie_delete');//KHi gọi route này nõ chuyển đến Controller có tên là MovieController và nó sẽ gọi hàm get_Movie_Delete để xử lý việc xóa một phim.
    });
    //Thiết lập đường đãn đến Schedule
    // Phần giải thích những route dưới đây sẽ tương tự như ở trên.
    Route::group(['prefix' => 'schedule'], function() {
        Route::get('schedule_list', 'ScheduleController@getScheduleList');
        Route::get('schedule_add', 'ScheduleController@getScheduleAdd');
        Route::post('schedule_add', 'ScheduleController@postScheduleAdd');
        Route::get('schedule_edit/{id}/{marap}/{giochieu}', 'ScheduleController@getScheduleEdit');
        Route::post('schedule_edit/{id}/{marap}/{giochieu}', 'ScheduleController@postScheduleEdit');
        Route::get('schedule_delete/{id}/{marap}/{giochieu}', 'ScheduleController@ScheduleDelete');
    });
