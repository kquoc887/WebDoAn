...
?>
                       ...
                       <li style="margin-left: 15px">
                          ...
                          <button type="button" style="margin-top: 5px ; margin-left: -3px" id="search"><i class="fa fa-search" style="width:25px; height:20px" ></i></button>
...
<div class="section third-section">
    <div class="container newsletter">
        ...
        <div id="success-subscribe" class="alert alert-success invisible">
            <strong>Cảm ơn!</strong>Quý khách đã đặt vé từ chúng tôi.</div>
            <div class="row-fluid">
                <div class="span5">
...
//if(strtotime($dong2["NGAYCHIEU"]) >= $today && strtotime($dong2["GIOCHIEU"])>strtotime(date("h:i")))
	...
	if ($ketqua) {
		foreach ($ketqua->fetchAll() as $dong) {
			...
			} else if (strtotime($another_date) == $today && strtotime($another_time) > $time) {
				$truyvan1 = 'SELECT MAPHIM,TENPHIM FROM movie WHERE MAPHIM = ' . $dong["MAPHIM"] . ';';
				$ketqua1 = $db->query($truyvan1);
				foreach ($ketqua1->fetchAll() as $dong1) {
	...
	foreach ($arrMaPhim as $key) {
		// duyệt mảng và xuất phim ra
